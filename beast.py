...
class Game(object):
    ...
    def __init__(self):
        ...
        win.border(0)
        win.nodelay(1)
        win.addstr(0, 3, " Hypernode Survivor v0.1 ")
    ...
    def print_actor(self, loc, actor):
        self.print_ch(loc, ICONS[actor], curses.color_pair(getattr(Colors, actor)) | curses.A_BOLD)

    def mainloop(self):
        self.loop_id = 0  # used to maintain player vs monster speed difference, as monsters only get to move on MOD x loops
        while True:
            self.loop_id += 1
            self.win.refresh()
            self.win.timeout(0)
            self.move_hero()
            if not self.loop_id % MONSTER_DELAY:
                self.move_monsters()
            time.sleep(TICK_TIME / 1000.0) # bah, int division default under 2.7
...
#        self.win.addstr(msg)
    ...
    def adjacents(self, y, x):
        ...
        for delta_x in range(-1, 2):
            for delta_y in range(-1, 2):
                ...
                # skip out of bounds
                if y + delta_y in (0, self.dim_y - 1): continue
                if x + delta_x in (0, self.dim_x - 1): continue
    ...
    def wrap(self, loc):
        ...
        if x == 0: x = self.dim_x - 2
        if x == self.dim_x - 1: x = 1
        if y == 0: y = self.dim_y - 2
        if y == self.dim_y - 1: y = 1
    ...
    def add_loc(self, q, p):
        ...
        return self.wrap(tuple(map(sum, zip(q, p))))
